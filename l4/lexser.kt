...
class SymbolTable {
    ...
    fun addVariable(name: String, type: Type, lineNumber: String, isArray: Boolean = false, forLoopVariable: Boolean = false) {
        table[name] = VariableInfo(type, true, lineNumber, isArray, mutableSetOf(), forLoopVariable)
    }
...
class SemanticAnalyzer(private val symbolTable: SymbolTable) {
    private var currentLineNumber = "0.0"  // Хранить как String
    ...
    fun analyze(tokens: List<Token>): List<SemanticError> {
        var i = 0
        while (i < tokens.size) {
            when (tokens[i].type) {  // Используем type вместо value
    ...
    private fun analyzeLet(tokens: List<Token>, index: Int) {
        ...
        val target = tokens[index + 1]
        when (target.type) {
            "IDENTIFIER" -> analyzeVariableAssignment(target.value)
            "ARRAY" -> analyzeArrayAssignment(target.value)
            else -> addError("Недопустимая цель для присваивания: ${target.value}")
        }

        val exprType = parseExpression(tokens.subList(index + 3, tokens.size), tokens).first.getType(symbolTable, this)
        val targetType = when (target.type) {
            "IDENTIFIER" -> symbolTable.getVariable(target.value)?.type
            "ARRAY" -> Type.NUMBER
            else -> null
        }
        ...
        expression = when (token.type) {
            ...
            "OPERATOR" -> {
                ...
                if (right is Expression.Error) {
                    return Pair(right, i)  // Propagate error
                }
                i += rightEndIndex
...
fun tokenize(codeLines: List<String>): List<Token> {
    ...
    val functionRegex = Regex("([A-Z]+)\\(([^()]*)\\)") // Более точное соответствие аргументам
    // This regex MUST match the entire array access, including the parentheses and index
...
class EnhancedParser(private val tokens: List<Token>) {
    ...
    private fun parseAssignment() {
        startNode("LET")
        index++
        parseVariable() // Изменено для поддержки массивов
        if (match("OPERATOR", "=")) {
            addNode("OPERATOR: =")
            parseExpression()
        }
    ...
    private fun parseExpression(): Unit {
    ...
    private fun parseFactor() {
        ...
        when {
            ...
            tokens[index].type == "ARRAY" -> parseArrayAccess() // Only call when it's an ARRAY token
            else -> index++
        }
        endNode()
    }
    ...
    private fun parseFunctionCall() {
        ...
        if (match("SYMBOL", "(")) {
            addNode("SYMBOL: (")
            index++
            parseExpression() // Разбор аргументов (упрощенно, без разделения запятыми)
            if (match("SYMBOL", ")")) {
                addNode("SYMBOL: )")
                index++
            }
